...
# после подъёма принудительно ПЕРЕСОЗДАЁМ nginx (не restart!):
#   - --force-recreate поднимает контейнер заново → он перечитает IP апстримов, И
#   - re-render шаблона: envsubst официального образа гоняется только при СТАРТЕ
#     контейнера (entrypoint), а `docker restart` его НЕ прогоняет. То есть после
#     rsync нового default.conf.template простой restart отдал бы старый конфиг —
#     изменения nginx-шаблона (напр. Host для MinIO) не применились бы до ручного
#     пересоздания. force-recreate закрывает обе проблемы разом.

...
# получен через getStickerSet — Bot API рендерит его нетронутым текстом ✦ у клиентов
# без Premium, поэтому HTML-фолбэк не меняется). parse_mode=HTML во всех _send*.
...
TEXT_CONFIRMED = (
    ...
    "отдельно. До встречи! ⚓️"
)
TEXT_WAIT_DECISION = "Твоя заявка на рассмотрении. Вернёмся с решением — жди здесь. ⚓️"

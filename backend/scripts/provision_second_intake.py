...
NEWS_BODY = (
    "⚓ Открыт набор во второй поток Экспедиции.\n"
    "\n"
    "Первым делом — познакомиться с платформой. Загляни в Базу знаний (/kb) и "
    "прочитай «Манифест» — не для галочки, дай себе время, пропитайся. Посмотри и "
    "Поддержку (/support) — там FAQ про основные возможности платформы.\n"
    "\n"
    "В разделе Задачи (/tasks) тебя ждут два приветственных задания: назвать себя "
    "и снять привычный образ — и заявить, зачем ты здесь.\n"
    "\n"
    "Пока стоим у причала — Экспедиция ещё не началась. Самое время освоиться на "
    "корабле."
)
...
FAQ_ITEMS: list[tuple[str, str]] = [
    ...
    (
        ...
        "В личном кабинете → «Оформление». По умолчанию тёмная, есть светлая "
        "(«пергамент»).",
    ),
    (
...
TASK_WELCOME2_TITLE = "Определи свою цель"
TASK_WELCOME2_BODY = (
    "Какое незавершённое дело висит у тебя в операционке более 3х месяцев? За "
    "время Экспедиции ты его либо выкинешь за борт, либо доделаешь."
)

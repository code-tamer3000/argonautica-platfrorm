...
Видео здесь НЕ трогаем: их постер снимает клиент при загрузке (тянуть видеофайлы
на бэкенд ради кадра дорого); у старых видео превью просто не будет — лента
покажет их как раньше.

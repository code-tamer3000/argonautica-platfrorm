...
# открывают раздел «Аргонавты» целиком (`app/api/argonauts.py` `_deny_cheap_tariff`,
# в ростере других участников их плитка при этом ЕСТЬ, хвостовой секцией) и видят
# только свой личный дневник, а не «Все дневники» (ARG-114) — см. `is_cheap_tariff`
# ниже.
...
def diary_visible(owner: User, viewer: User) -> bool:
    ...
    Дневник admin-владельца в чужом «Все дневники» по умолчанию не показываем —
    Динамика не для админов, но `create_user` заводит личный канал любому новому
    аккаунту без учёта роли, так что такой дневник может существовать. Исключение —
    `owner.diary_public`: ручной разовый флаг конкретному админу («показать участникам
    СВОЕГО потока», не общее правило) — остальные админы остаются невидимы. Не влияет
    на просмотр СВОЕГО дневника (та ветка в assert_room_access/list_rooms не зовёт эту
    функцию)."""
    if owner.role == "admin":
        return owner.diary_public and owner.intake_id == viewer.intake_id

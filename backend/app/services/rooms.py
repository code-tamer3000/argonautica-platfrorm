...
async def prune_dm_memberships_after_plan_change(session: AsyncSession, user: User) -> None:
    ...
    (односторонний уход из dm, история у него остаётся как обычно; при повторном
    dm с этим же собеседником `dm_key` создаст комнату заново по общей логике
    создания dm). Дешёвый тариф — вызывающая сторона (`update_user`), но правило
    общее: применяется на любую смену `plan_id`, не только на понижение.
    """
    room_ids = (
        await session.execute(

...
class PlaylistListOut(BaseModel):
    """Список плейлистов, ДОСТУПНЫХ смотрящему, — для пикера «прикрепить уже
    существующий» (docs/FILES.md «Плейлист»). Область та же, что у чтения одного
    плейлиста не-автором: свои + те, что прикреплены к носителю, который юзер и
    так видит. Плейлист из чужой лички в список не попадает."""

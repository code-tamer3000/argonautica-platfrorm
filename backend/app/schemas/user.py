...
class AdminUpdateUserRequest(BaseModel):
    ...
    # Смена тарифа задним числом (напр. понижение/восстановление). При смене
    # эндпоинт синхронизирует dm-членства с новым видимым кругом в обе стороны
    # (см. resync_dm_memberships_after_plan_change) — остальные ограничения
    # (дневники, контакты, задачи/КБ по тарифу) пересчитываются живьём на каждый
    # запрос, кроме уже сданных common-задач — те остаются видны независимо от
    # тарифа (см. docs/TASKS.md "Tariff change cleanup").
    plan_id: int | None = None

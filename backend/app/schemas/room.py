...
class RoomOut(BaseModel):
    ...
    # Только is_personal: АКТИВНЫЙ поток владельца дневника (не архив) — фронту
    # нужен для группировки «Все дневники» на текущий/архивные разделы и для
    # клиентского фильтра admin «текущая экспедиция» (RoomList.tsx). Не путать с
    # правом видеть дневник вообще — то решает сервер (diary_visible).
    owner_intake_id: int | None = None

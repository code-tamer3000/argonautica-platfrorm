...
- `GET /api/dynamics/my-stats` — closed/partial day counts, streak / misses / a ±day window, computed from personal-room messages + pardons/credits. Also `overdue_tasks` (list) and `late_submissions_count` — task-overdue metrics reused from Tasks (ARG-130/ARG-128), shown alongside diary misses in `ProfileScreen`'s Dynamics block; see [TASKS.md](TASKS.md) "Просрочка задачи как метрика".
...
- Admin dynamics: `GET /api/admin/dynamics` (summary + per-participant rows, each with a ±window `recent_days`, plus `overdue_tasks_count`/`late_submissions_count` and `limbo_eligible` — see [LIMBO.md](LIMBO.md) "Explicit manual trigger"), `GET /api/admin/dynamics/{user_id}/days` (that one participant's full 28-day period — used when a card is expanded in the admin UI, since the list response only carries the ±window), `POST /api/admin/dynamics/credit` (grant/revoke a day).

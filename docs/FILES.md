...
**Edit after send (author only, ARG-139 post-review).** Two mutations, gated by `assert_playlist_owner` (`playlist.created_by == current_user.id`, independent of the carrier's own permissions): `PATCH /api/media/playlists/{id}` renames the title; `DELETE /api/media/playlists/{id}/tracks/{track_id}` drops one track (400 if it's the last one — a playlist can't go empty; delete its carrier instead). Track order, track metadata and the track set otherwise stay a snapshot of attach time — no reorder, no add-track, no per-track rename API.

**Lock-screen / notification controls.** `GlobalPlayer` sets `navigator.mediaSession.metadata` (title/artist/album/artwork) per track and registers `play`/`pause`/`previoustrack`/`nexttrack`/`seekto`/`seekbackward`/`seekforward` handlers, so the phone's system media notification shows the actual track (not the PWA's generic name) and its transport buttons drive the same store as the in-app player.

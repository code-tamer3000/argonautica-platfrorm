...
**Boundaries (see ARG-139 for the full list).** No shared track library — a playlist's tracks are exactly what its author attached, nothing is reused across playlists. Immutable after send: no rename/reorder/add-track API once attached (only deletable with its carrier). No drag-and-drop reordering in the composer — order is attachment order. No Media Session API / background playback / HLS for these tracks (still whole-file presigned playback, same as any other audio).

...
**Cover art offline cache (fix).** The `Blob` caching above only ever covered *tracks* — the cover art shown in `PlaylistCard`, the mini/expanded `GlobalPlayer`, and `DownloadedPlaylistsSection` was always the raw presigned `cover_url`, which cannot load at all without a network round-trip. Truly offline (not just the cold-start signature race ARG-153's `online`-retry handled), the cover permanently fell back to the `IconMusic` placeholder even for a fully downloaded playlist. Fixed by reusing the byte-level preview cache from ARG-149/ARG-152 (`lib/attachmentPreviewCache.ts`, store `mediaPreviewCache`, keyed by URL pathname): all three render sites now resolve `cover_url` through `hooks/useOfflinePreviewSrc.ts`, which opportunistically caches the bytes on a successful load and swaps in the cached `Blob` URL on failure; `downloadPlaylist()` additionally calls `cachePreviewByFetch(cover_url)` explicitly, so the cover is guaranteed cached at download time rather than depending on the card having rendered it first.

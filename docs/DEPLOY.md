...
|---|---|---|---|
| `main` | Production | `45.151.102.255` (`platform.argonautica-systems.ru`) | merge → GitHub Actions (`deploy-prod.yml`) → rsync + `docker/deploy.sh` |
...
- Staging is isolated (separate compose project, own network/volumes/`.env`/`JWT_SECRET`), no blue-green, **no `bot` service** (a second long-poller on the prod token would break the prod bot — see [TELEGRAM_BOT.md](TELEGRAM_BOT.md)). Staging *does* run `intake-bot` (ARG-92, `backend/scripts/intake_bot.py`) — a separate intake/payment funnel bot on its **own** Telegram token, so it doesn't touch the prod bot's `getUpdates` poller; the two are unrelated services.
...
- **Domain & TLS:** staging answers only on `staging.argonautica-systems.ru` (access by raw IP is closed by the prod catch-all `default_server`). A-record → `45.151.102.255` (same host as prod, same port `:443` — one gateway serves both, routed by SNI/Host, not by port). Real **Let's Encrypt** cert, issued/renewed via **webroot through prod's `:80`** (prod nginx already serves `/.well-known/acme-challenge/` from the shared `docker_certbot_webroot` volume), same as before — but the cert itself now lives with **prod**, in `docker/nginx/certs/${STAGING_DOMAIN}.{crt,key}`, since prod-nginx is the one terminating TLS for the staging domain. The **host** certbot (v0.40) is broken, so issuance/renewal run in the `certbot/certbot` **container**. Renewal + delivery: `docker/nginx/renew-staging-cert.sh` (installed on the server as `/opt/platform/renew-staging-cert.sh`, cron twice-daily) renews, copies the cert into `docker/nginx/certs/`, and **recreates prod nginx** (not staging nginx).
...
- **Content + container:** the page's own files live on the server in `/root/argonautica-site` (**outside this repo** — separate site, separate release rhythm), mounted read-only into an `nginx:alpine` container `argonautica-site`. The container is declared in `docker/argonautica-site.compose.yml` as its **own compose project**, so `deploy.sh` never sees it and a blue-green deploy of the platform cannot restart or break it. Its nginx config is tracked here: `docker/argonautica-site/nginx.conf`. Bring it up (from `/opt/platform`, `SITE_ROOT` overrides the content path):
...
- **Price sync:** the site's tariff cards read `/prices.json` (client-side `fetch`, own repo) so editing a plan's price in the platform admin reaches the site without a redeploy. The site container has no network path to the platform (only `gateway`, not `docker_default`), so `docker/argonautica-site/sync-prices.sh` runs **on the host** via cron, reads `plans` with `docker exec` into the Postgres container, and writes `prices.json` straight into `SITE_ROOT` (same volume nginx already serves) — no new port, no public API. Writes **every** tariff, not just active ones — each keyed by name to `{price, is_active}` — so the site can render a deactivated tariff as unavailable instead of the card just vanishing; the site's own code (separate repo) has to be updated to read this shape. Installed manually per environment, e.g.:
  ```bash
  * * * * * PG_CONTAINER=docker-postgres-1 SITE_ROOT=/root/argonautica-site /opt/platform/docker/argonautica-site/sync-prices.sh
  */2 * * * * PG_CONTAINER=platform-staging-postgres-1 SITE_ROOT=/root/argonautica-site-preview /opt/platform-staging/docker/argonautica-site/sync-prices.sh
  ```
  Site side keeps the last successfully fetched prices in `localStorage` as the fallback shown before the next fetch resolves (or if it fails) — so a stale hardcoded default is only ever the very first load.

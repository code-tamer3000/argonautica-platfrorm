...
The chat is built to survive bad networks — nothing typed or sent is lost. All of it is client-only, backed by a small dependency-free IndexedDB wrapper (`lib/idb.ts`, stores `outbox`/`drafts`/`querycache`).

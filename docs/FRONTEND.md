...
React 18 + TypeScript + Vite. TanStack Query v5 (server state), Zustand (UI state), react-router-dom v6. Markdown (`marked` + `DOMPurify`) is used in the knowledge base, tasks (curated, admin-authored bodies), and **journal channels** (`room.type === 'channel' && !is_news` — «Дневник»/«Личный дневник», where participants keep daily formatted entries), via `lib/markdown.ts`. DM, group and news-channel messages render as **plain text** (`lib/messageText.tsx`): newlines preserved, bare URLs auto-linked, @-mentions highlighted, no full markdown syntax — nobody formats a DM with `#`/lists, and rendering it there just swallowed literal characters. The one exception is three inline marks — bold/italic/underline — applied WYSIWYG in the composer itself (`Composer.tsx`'s message field is `contentEditable`, not a `<textarea>`; formatting goes through `document.execCommand` via `useRichFormatting.tsx`, no raw `**`/`*`/`++` visible while typing) and serialized to those markers only on submit (`lib/inlineMarks.ts`'s `htmlToMarkerText`/`markerTextToHtml` — the two-way DOM ⇄ marker-text conversion; a draft or admin-injected pending text is deserialized back into the field the same way). Both render paths for already-sent messages understand the same markers, `messageText.tsx` via its own recursive tokenizer (React nodes, no `dangerouslySetInnerHTML`), `markdown.ts` via a small `marked` inline extension for `++underline++` (markdown has none natively). @-mention autocomplete (`useMentionAutocomplete.tsx`) works off the Selection/Range API against the contentEditable DOM, not `textarea.selectionStart`. No test runner yet — the gate is `tsc --noEmit`.
...
- `components/` — shared UI (Avatar, Badge, Button, Card, Chip, ConfirmDialog, EmptyState, Input, KebabMenu, PageHeader, Segmented, Spinner, Overlay=Modal/Drawer/Lightbox, Toasts, icons, VideoPlayer, MediaComposer).
- **Chat composer paperclip → menu** (not a direct file dialog): «Файл» (upload as before) or «Ссылка на материал / задачу» → `features/chat/RefPicker.tsx` (tabs Материалы/Задачи, title search over `useKbItems`/`useTasks`, which the server already scopes to visible items — plus, for admin, the same `adminCurrentIntakeId` narrowing as `KbList`/`TasksList`, since the server does not gate these lists by intake for admin at all). Materials are grouped by KB category (same grouping pattern as `KbList`). One picked ref → a chip above the input (beside media chips) and `ref_kind`/`ref_id` on `SendBody`; the optimistic bubble shows the ref immediately (`OutboxItem.optimisticRef`). `MessageItem` renders `msg.ref` as a «Перейти к материалу/задаче» button before the text (disabled when `available=false`). Media and a ref can ride the same message. See [MESSAGES.md](MESSAGES.md).
...
**Destructive actions (two-step confirm).** `components/ConfirmDialog.tsx` wraps `Modal`
with a title/text and Отмена/Удалить buttons — use it instead of `window.confirm` for new
delete flows (most existing call sites still use `window.confirm`; only Задачи and База
знаний were converted so far, see below). `components/KebabMenu.tsx` is the generic
«three dots» dropdown for per-row/per-card admin actions (edit/publish/delete), following
the same click-outside idiom as `features/app/ProfileMenu.tsx`; it calls
`e.preventDefault()` on its own clicks so it can sit inside a `<Link>`-based card without
triggering navigation. Both were introduced when the `/admin/tasks` and `/admin/kb`
admin-panel screens were folded into `features/tasks/TasksList.tsx` and
`features/kb/KbList.tsx`/`KbViewer.tsx` directly (admin actions gated by
`user?.role === 'admin'`, no more duplicate `/admin/*` screen) — see [TASKS.md](TASKS.md)
and [KB.md](KB.md).
...
- **Bootstrap cache persist** (`lib/queryPersist.ts`). A stable slice of the TanStack Query cache (rooms/users/stickers/messages/dashboard/kb/tasks/dynamics/notifications/media/argonauts — `PERSIST_KEYS`, matched by the first `queryKey` segment) is dumped to IndexedDB (debounced) and restored **before** first render in `main.tsx`, so a repeat visit paints instantly from the last session; restored data is marked stale (`updatedAt: 0`) so focus/mount triggers a background refetch. `refetchOnWindowFocus`/`refetchOnReconnect` are now on — the tab-switch "disappearing message" bug was stale cache never refetching on focus. `dashboard` (ARG-146) — the home screen ("Круг Экспедиции") is the landing page, so it needs to paint from cache on a cold offline start same as rooms/messages do. `kb`/`tasks`/`dynamics`/`notifications` (ARG-147) extend the same cold-offline-start coverage to КБ/Задачи/Динамику/Уведомления; Новости need no extra key — an opened news item is an ordinary room/message (`is_news`), already covered by `rooms`/`messages`. Каюта is deliberately excluded — it's the only section with real privacy (see [CABIN.md](CABIN.md)); the device gate doesn't encrypt IndexedDB, so its data never enters this persist layer. `media` — the per-`assetId` presigned-URL lookup (`useMediaUrl`, `api/media.ts`) that KB attachments resolve through instead of getting the URL inline in a feed payload; without it the URL itself never survived offline and the attachment stuck on "loading" even though the JSON containing it would otherwise be cacheable. `argonauts` — the roster/profile page (`api/argonauts.ts`) had no persist coverage at all until this was noticed while chasing the `media` bug.
...
**Media cache, two layers (ARG-75 browser HTTP cache; a byte-level IndexedDB layer added by ARG-149; the SW layer from ARG-16 stays removed — see ADR-032).**

- **Ordinary browser HTTP cache**, works for **all** media kinds on **every** origin including `MEDIA_DOMAIN`. The backend rounds the presigned-GET signing moment down to a 24h window (`PRESIGN_GET_WINDOW`, `services/media.py`, see [FILES.md](FILES.md)), so repeat requests for the same object within the window get a byte-identical URL — the browser cache, which keys on the full URL including query, hits. nginx's `Cache-Control: private, max-age=86400, immutable` was always correct; it just had nothing to key on before. No SW/Cache Storage involved, so this layer benefits video/audio too (though 206-range responses cache poorly in Chrome/Firefox's HTTP cache — the gain there is partial; full range/segment delivery is [ARG-77]).
- **Removed: SW `CacheFirst` second layer** (`sw.ts` + `lib/mediaCache.ts`, cache `arg-media-v2`). Predated the HTTP-cache fix above (ARG-16) and was kept as a "second line of defense" after ARG-75 made it largely redundant — until it turned out to be an active liability instead: it caused two separate real-world iOS PWA bugs in production (a transient network failure getting cached as permanent, per-object, until manual cache clear; and a request issued right at PWA cold-start racing the Service Worker's own startup and hanging forever with no timeout, reproducible as "image never loads on first open of a session, works instantly on every open after that until the app is force-killed"). Removed rather than patched further — the plain HTTP cache above already covers the case it existed for. `MEDIA_CACHE_NAME`/`clearMediaCache()` remain in `lib/mediaCache.ts` and are still called on logout (`features/auth/api.ts`) purely to flush whatever old entries already-installed PWAs still have lying around from before the removal — media is private and the device may be shared; best-effort, so a missing Cache API can't break logout.
- **Added: `lib/attachmentPreviewCache.ts`, byte-level IndexedDB cache (ARG-149).** Reported manually: the HTTP-cache layer above did not survive a real "kill the process → go offline → cold relaunch" sequence for chat attachments even though the presigned URL itself was already persisted (`queryPersist.ts`'s `messages`/`rooms` keys carry the inline `thumb_url`/`preview_url`, but the *bytes* behind those URLs live only in the disk HTTP cache — this task did not re-run the CDP-offline/real-airplane-mode repro to confirm the disk cache is the failure point rather than something else in that sequence; treat the diagnosis as the reporter's, not independently re-verified here). Fix (byte-level cache, independent of *why* the HTTP cache misses): keyed by URL **pathname** (the presigned signature rotates every 24h window, the object uuid in the path doesn't — same key trick the removed SW layer used, see ADR-032). Only `thumb_url` (feed) and `preview_url` (lightbox/video poster) are cached, never the full-size `url` (original/download); a component only writes to the cache after the bytes it's rendering actually loaded successfully — never a bulk background fetch of everything in the feed. Two read paths: `useOfflinePreviewSrc` (`hooks/useOfflinePreviewSrc.ts`) for plain `<img>`/`<video poster>` — probes the URL with a detached `Image()` (native `<video poster>` has no error event to hang a fallback off), swaps to the cached blob on failure, caches on success; and `useImageDownload` (lightbox preview, already streams the full blob for its progress bar) writes/reads the same store directly, no second fetch. Cap: 60 entries / 7 days (mirrors the removed SW layer's `workbox-expiration` config), oldest evicted first. Cleared on logout (`clearAttachmentPreviewCache()`, best-effort, alongside `clearMediaCache()`/offline playlists). **Not a Service Worker fetch interception** — plain `fetch`/`Image()` calls from the main thread, no Cache Storage, no request routing through `sw.ts`; the two ADR-032 failure modes (a failed fetch cached as permanent, a request racing the SW's own cold-start) don't apply because there's no interception layer to race or to get stuck. See ADR-032 for why that door stays shut for anything that *does* intercept.
...
**Albums & group lightbox.** A message with 2+ visual attachments renders one grid (`features/chat/MediaGroup.tsx`, `.album2`…`.album6`/`.albumMany` in `chat.module.css`) instead of a column of separate boxes; tiles are `thumb_url` under `object-fit: cover` and the grid's `aspect-ratio` reserves the height before any byte arrives. `Lightbox` (`components/Overlay.tsx`) takes either a single `url`/`kind` (unchanged call sites) or `items` + `index`, in which case it pages the group with arrows, ←/→, a horizontal swipe and an `n / total` counter; the current item is keyed so per-item load/buffer state restarts on switch. Both composers accept several files at once (`input multiple`): the chat composer caps the message at 6 (`MAX_ATTACHMENTS` in `lib/mediaUpload.ts`, mirroring the backend) and `MediaComposer` uploads the selection sequentially, showing «N из M» beside the progress bar.

...
> Частично внесено в `main` отдельно от прочих фич develop (каюта/стенд и др.
> остаются в develop). Нумерация стадии — как в develop.
...
- **Кэш** — presigned-GET TTL 24 ч (`PRESIGN_GET_EXPIRES`); nginx отдаёт
  ...
  ответы — прод-шаблон ([docker/nginx/templates](../docker/nginx/templates/default.conf.template)).

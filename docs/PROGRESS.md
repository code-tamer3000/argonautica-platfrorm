...
## Стадия 25 — Каюта: группировка дневника по дате (2026-07-05) ✅

Дневник эмоций теперь группирует записи по дате в раскрывающиеся секции (за одну
дату может быть несколько записей): заголовок карточки — триггерное событие, дата
вынесена в шапку группы. Авто-дата сохранена, убрана только дублирующая подсказка.

### Реализовано
- **Группировка** — новый компонент `CabinEntryList` + `DateGroup` в
  [CabinEntryCard.tsx](../frontend/src/features/cabin/CabinEntryCard.tsx): если у подраздела
  задан `groupBy` (у дневника — `date`), записи собираются в раскрывающиеся группы по
  значению поля с сохранением порядка (сначала новые); иначе — плоский список. Подключён
  и в личном экране [CabinScreen](../frontend/src/features/cabin/CabinScreen.tsx), и в
  админском [AdminCabin](../frontend/src/features/admin/AdminCabin.tsx).
- **Поля дневника** — в [cabinFields.ts](../frontend/src/features/cabin/cabinFields.ts)
  `titleField` дневника переключён на `trigger`, добавлен `groupBy: 'date'`; поле группировки
  не дублируется в теле карточки. Убрана подсказка «по умолчанию — сегодня» у поля «Дата»
  (авто-подстановка сегодняшнего числа через `todayStr()`/`today` сохранена).
- **Стили** — классы `.dateGroup/.dateHead/.dateLabel/.dateCount/.dateItems` в `cabin.module.css`.
...
## Стадия 26 — Каюта: доступ по выдаче + уведомление (2026-07-05) ✅

Раздел «Каюта» больше не доступен всем: по умолчанию закрыт, админ выдаёт доступ
конкретному участнику, а тот получает уведомление-колокольчик о выдаче.

### Реализовано
- **Флаг доступа** — `users.can_access_cabin` (BOOLEAN, default `false`) в
  [models/user.py](../backend/app/models/user.py); миграция
  [a2b3c4d5e6f7](../backend/alembic/versions/20260705_1400_a2b3c4d5e6f7_cabin_access_and_grant_notification.py)
  (add column + расширение CHECK по kind уведомлений + `notifications.room_id` → nullable,
  всё обратно-совместимо, п.8).
- **Серверная проверка** — зависимость `require_cabin_access` в
  [api/deps.py](../backend/app/api/deps.py) навешена на все личные эндпоинты
  `/api/cabin/{kind}` (admin — доступ всегда). Без доступа — 403.
- **Выдача + уведомление** — `can_access_cabin` добавлен в whitelist PATCH
  [api/admin.py](../backend/app/api/admin.py); на переходе false→true вызывается
  `notify_cabin_granted` ([services/notifications.py](../backend/app/services/notifications.py)) —
  системное уведомление `cabin_granted` (без комнаты/автора, kind добавлен в CHECK и Literal).
- **Фронт** — тумблер «Доступ к разделу „Каюта“» в
  [AdminUsers](../frontend/src/features/admin/AdminUsers.tsx); скрытие пункта навигации и
  маршрута `/cabin` без доступа в [AppShell](../frontend/src/features/app/AppShell.tsx)
  (`can_access_cabin || role==='admin'`); рендер уведомления `cabin_granted` в
  [NotificationBell](../frontend/src/features/app/NotificationBell.tsx) и тосте
  ([useRealtime](../frontend/src/hooks/useRealtime.ts)); клик ведёт в `/cabin`
  ([useOpenNotification](../frontend/src/features/app/useOpenNotification.ts)).

### Проверено
- `pytest` — 142 passed (в т.ч. +4 теста Каюты: закрыто по умолчанию, выдача открывает
  доступ и шлёт уведомление, идемпотентность выдачи). `ruff`, `mypy`, фронтовый
  `npm run build` — зелёные.

---

## Стадия 27 — Быстрая доставка медиа: превью + presigned в ленте + кэш (2026-07-05) ✅

Фото/видео в ленте грузились медленно (особенно с телефона) из-за латентности и числа
round-trip'ов, а не размера файлов. Три слоя оптимизации — см.
[DECISIONS.md](DECISIONS.md#быстрая-доставка-медиа-превью--presigned-в-ленте--кэш).

### Реализовано
- **Превью картинок** — `media_assets.thumb_key` ([models/media.py](../backend/app/models/media.py)),
  миграция [b3c4d5e6f7a8](../backend/alembic/versions/20260705_1600_b3c4d5e6f7a8_media_thumb_key.py)
  (add column nullable, обратно-совместимо, п.8). Генерация — `generate_image_thumbnail`
  (Pillow, WebP ≤1024px, best-effort) при подтверждении загрузки
  ([services/media.py](../backend/app/services/media.py), [api/media.py](../backend/app/api/media.py));
  зависимость `pillow` добавлена в [pyproject.toml](../backend/pyproject.toml).
- **Presigned-URL прямо в ленте** — `MessageOut.attachments` (схема `AttachmentOut`:
  url + thumb_url + метаданные); батч-подпись `resolve_attachments`
  ([services/media.py](../backend/app/services/media.py)) во всех эндпоинтах сообщений и
  в WS-broadcast ([api/messages.py](../backend/app/api/messages.py)). `attachment_ids`
  оставлен для совместимости.
- **Кэш** — presigned-GET TTL 24 ч (`PRESIGN_GET_EXPIRES`); nginx отдаёт
  `Cache-Control: private, max-age=86400, immutable` на медиа и `gzip` на текстовые
  ответы — прод и staging шаблоны ([docker/nginx*/templates](../docker/nginx/templates/default.conf.template)).
- **Фронт** — `Attachment` рендерит вложение из ленты без per-asset запроса, в ленте
  грузит превью, оригинал — в лайтбоксе по клику
  ([Attachment.tsx](../frontend/src/features/chat/Attachment.tsx),
  [MessageItem.tsx](../frontend/src/features/chat/MessageItem.tsx)); типы + staleTime
  ([lib/types.ts](../frontend/src/lib/types.ts), [api/media.ts](../frontend/src/api/media.ts)).
  Путь по `assetId` (база знаний) сохранён как фолбэк.

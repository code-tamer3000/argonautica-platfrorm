...
- **Cohort-gated only, no rank cascade (ARG-96 intake gate; ARG-112 removed the ARG-110 rank cascade for diaries).** A diary room's own `intake_id` stays NULL on purpose (it's tied to a user, and the user already carries `intake_id`) — so visibility of an *other* user's diary is computed by comparing the diary owner's and the viewer's `user_intake_scope` (`diary_visible` in `app/services/visibility.py`), not through the room's own columns or `room_plans`. The **only** requirement is a shared intake in that scope — the owner's tariff plays no role (ARG-110 originally gated this by tariff rank too; ARG-112 dropped that on purpose, so all tariffs of a stream see each other's diaries). The owner always sees their own diary regardless; admin sees every diary. Checked in `assert_room_access` (personal branch) and mirrored in `list_rooms`' query filter (`others_personal_visible`), so an out-of-reach diary is both invisible in the list and 403 on direct `GET /api/rooms/{id}`. **An admin-owned diary is shown to a participant only through the admin's own archive** (`diary_visible` intersects the viewer's scope with the admin owner's `user_intake_access` rows, never the admin's active intake) — Dynamics isn't an admin feature, but `create_user` provisions a personal room for every new account regardless of role, so one can exist; without an archive grant it stays out of participants' «Все дневники» entirely (`diary_visible` is only consulted for a non-admin *viewer* to begin with — see the personal branch of `assert_room_access`). It stays fully visible to **other admins** (unrestricted oversight, unaffected by `diary_visible`), grouped into its own «Админ» section (see below) rather than reading as tariff-less. An admin always sees their own diary the same way any owner does.
- **Multi-intake archive (`user_intake_access`, [DATA_MODEL.md](DATA_MODEL.md)).** A participant moved to a new intake (`PATCH /api/admin/users/{id}` `intake_id`) instantly loses «Все дневники» of the old cohort — the move is a plain overwrite, no history kept. An admin grants back READ-ONLY access to a specific past intake's diaries (and KB items, see [KB.md](KB.md)) via `archive_intake_ids` on the same PATCH endpoint (full-replace of the row set). `diary_visible` and the `list_rooms` SQL mirror both compare `user_intake_scope` (active intake ∪ archive rows) instead of the bare `intake_id`, so this is symmetric: the moved user regains the old cohort's diaries, and the old cohort regains the moved user's diary — provided the moved user (not the peer) has the archive grant. Comments (thread replies) into a diary reached only through archive access are blocked (`app/api/messages.py` `send_message` checks the owner's ACTIVE `intake_id` against the writer's, ignoring archive) — read-only by design; the owner can always write into their own diary. Client-side, `RoomOut.owner_intake_id` (the owner's active intake) plus `AdminUserOut.archive_intake_ids` let `RoomList.tsx` keep an admin's "current expedition" filter from hiding a moved user's diary, and `UserOut.archive_intakes` (id + `starts_on`) let a participant's own client split «Все дневники» into the current cohort's section and one «Архив · <date>» section per granted past intake.
- **Except the cheapest tariff, cut on BOTH sides independently (ARG-114 viewer side, ARG-117 owner side).** `is_cheap_tariff(session, user)` (`app/services/visibility.py`) — a viewer holding `CHEAP_TARIFF_NAME` (the same plan name that `OBSERVER_TARIFF_NAME` in `app/api/argonauts.py` re-exports, already excluded from the «Аргонавты» roster) never sees anyone *else's* diary, regardless of intake match (ARG-114) — checked alongside `diary_visible` in `assert_room_access`'s personal branch, and mirrored in `list_rooms`'s `others_personal_visible` (forced `false()` for such a viewer instead of the usual intake-`exists()` clause). Symmetrically, a diary OWNED by a `CHEAP_TARIFF_NAME` holder is invisible to everyone but the owner and admins (ARG-117) — same two call sites, opposite direction: `assert_room_access` additionally checks `is_cheap_tariff(session, owner)` (that whole branch already runs only for a non-admin viewer, so no extra role check needed), and `list_rooms` adds a negated correlated `exists().where(Plan.id == Owner.plan_id, Plan.name == CHEAP_TARIFF_NAME)` to the `others_personal_visible` EXISTS clause — deliberately NOT a single resolved-once `cheap_plan_id` compared with `==`: `plans.name` carries no uniqueness constraint (harmless in prod where only one such plan is ever created, but the test suite creates a fresh same-named plan per test — a single "the" id would silently pick an arbitrary one and miss the owner's actual plan row). Neither check touches `diary_visible` itself — the *owner's* tariff still plays no role for every OTHER plan (ARG-112's original rule stands); only this one plan is special, on both the viewer and the owner side, independently. Both checks are independent of `is_observer` (which blocks the whole Рубка, own diary included). The cheap-tariff holder's own diary is unaffected either way — they still see it, and so do admins regardless of whose diary it is. `UserOut.is_cheap_tariff` (`GET/PATCH /api/auth/me`) exposes the flag so the frontend can skip straight to the owner's own diary — `ChatLayout.tsx` redirects `/diaries` to `/diaries/{own room id}` for such a viewer instead of rendering the (now perpetually empty for them) «Все дневники» list.
- **Grouped by tariff on the client (display only, not a visibility filter).** `RoomOut.owner_plan_id`/`owner_plan_name` carry the diary owner's plan (batch-joined in `list_rooms`/`get_room`, `app/api/rooms.py`) — `RoomList.tsx` groups «Все дневники» into one `sectionHeader` per tariff via `groupDiariesByPlan` (`features/chat/util.ts`, thin wrapper over the generic `groupByPlan` in `lib/planGroups.ts`), ordered the same way `GET /api/plans` returns them (all tariffs including deactivated ones, price-ascending — the public, any-authenticated-user counterpart of admin-only `/api/admin/plans`, id+name only; unlike the intake bot's tariff list, this one does not filter on `is_active`, see [DATA_MODEL.md](DATA_MODEL.md) `plans`). `groupByPlan` buckets by `owner_plan_id`, sorts by that same price order, and always puts a `NULL`-id bucket ("Без тарифа") last. An admin owner is deliberately given a **sentinel** `owner_plan_id = -1` (`_ADMIN_OWNER_PLAN_ID` in `app/api/rooms.py`, not a real row in `plans`) alongside `owner_plan_name = "Админ"` — bucketing by name instead would've silently merged every admin diary into "Без тарифа" (`groupByPlan` keys on `id`, not `name`); the sentinel earns it its own section, sorted after every real tariff and before "Без тарифа" (not in the `plans` price map, so it falls through to `groupByPlan`'s "unknown id" tail-before-NULL fallback). Since ARG-112, this grouping is purely cosmetic — every tariff's section is populated the same way for every viewer of the stream, nothing is filtered out by it.

...
- **Cohort-gated only, no rank cascade (ARG-96 intake gate; ARG-112 removed the ARG-110 rank cascade for diaries).** A diary room's own `intake_id` stays NULL on purpose (it's tied to a user, and the user already carries `intake_id`) — so visibility of an *other* user's diary is computed by comparing the diary owner and the viewer directly (`diary_visible` in `app/services/visibility.py`), not through the room's own columns or `room_plans`. The **only** requirement is the same `intake_id` — the owner's tariff plays no role (ARG-110 originally gated this by tariff rank too; ARG-112 dropped that on purpose, so all tariffs of a stream see each other's diaries). The owner always sees their own diary regardless; admin sees every diary. Checked in `assert_room_access` (personal branch) and mirrored in `list_rooms`' query filter (`others_personal_visible`), so an out-of-reach diary is both invisible in the list and 403 on direct `GET /api/rooms/{id}`. **An admin-owned diary is hidden from participants by default** (`diary_visible` returns `owner.diary_public and owner.intake_id == viewer.intake_id` for an `admin` owner, `False` unless both hold) — Dynamics isn't an admin feature, but `create_user` provisions a personal room for every new account regardless of role, so one can exist; this just keeps it out of participants' «Все дневники» by default (`diary_visible` is only consulted for a non-admin *viewer* to begin with — see the personal branch of `assert_room_access`). `users.diary_public` (`PATCH /api/admin/users/{id}`, admin-only field like `is_navigator` — 400 if set on a non-admin) is a manual, per-admin opt-in: turning it on shows *that one* admin's diary to participants of *their own* intake only, everyone else's admin diary stays hidden. It stays fully visible to **other admins** regardless of the flag (unrestricted oversight, unaffected by `diary_visible`), grouped into its own «Админ» section (see below, always rendered first) rather than reading as tariff-less. An admin always sees their own diary the same way any owner does.

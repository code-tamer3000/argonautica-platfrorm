...
- **Except the cheapest tariff, whose *viewer* side is cut (ARG-114).** `is_cheap_tariff(session, user)` (`app/services/visibility.py`) — a viewer holding `CHEAP_TARIFF_NAME` (the same plan name that `OBSERVER_TARIFF_NAME` in `app/api/argonauts.py` re-exports, already excluded from the «Аргонавты» roster) never sees anyone *else's* diary, regardless of intake match — checked alongside `diary_visible` in `assert_room_access`'s personal branch, and mirrored in `list_rooms`'s `others_personal_visible` (forced `false()` for such a viewer instead of the usual intake-`exists()` clause). This is independent of `is_observer` (which blocks the whole Рубка, own diary included) and doesn't touch `diary_visible` itself — the *owner's* tariff still plays no role, per the point above; only the *viewer's* does, and only for this one plan. The viewer's own diary is unaffected. `UserOut.is_cheap_tariff` (`GET/PATCH /api/auth/me`) exposes the flag so the frontend can skip straight to the owner's own diary — `ChatLayout.tsx` redirects `/diaries` to `/diaries/{own room id}` for such a viewer instead of rendering the (now perpetually empty for them) «Все дневники» list.

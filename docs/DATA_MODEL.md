...
| duration | INT | NULL | seconds, video/audio |
| transcode_status | TEXT | NULL, CHECK in (`processing`,`done`,`failed`) | server video/audio transcode state; NULL = not video/audio / legacy (pre-feature) |
| variant_key | TEXT | NULL | served variant key: video → H.264 720p mp4 (`video/720/<uuid>.mp4`), audio → AAC/M4A (`audio/aac/<uuid>.m4a`); = `storage_key` on the fast path |
| variant_mime | TEXT | NULL | variant mime (`video/mp4` or `audio/mp4`) |
| created_by | BIGINT | FK users, NOT NULL | |
| created_at | TIMESTAMPTZ | NOT NULL | |
...
| is_draft | BOOLEAN | NOT NULL DEFAULT false | draft: visible to admins only until cleared (task-side equivalent of `kb_items.published`, [TASKS.md](TASKS.md) «Черновик»). Orthogonal to `publish_at` — a draft stays hidden even when its date has passed; checked by the same `published_where`/`is_published` |
...
| Video/audio transcode queue | `transcode:pending` (list), `transcode:inflight` (hash, claim ts), `transcode:attempts` (hash) — shared by video and audio jobs, dispatched by `kind`. Job state + retry count; durable serving state is `media_assets.transcode_status`. See [FILES.md](FILES.md) |

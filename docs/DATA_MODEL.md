...
- *Чтение*: бэкенд выдаёт **presigned-GET** на время. Видео — с поддержкой HTTP
  range-запросов (перемотка), MinIO это умеет.
...
- **Метрики сервера** — предыдущий снимок счётчиков CPU/сети (`admin:metrics:prev`)
  для расчёта мгновенных скоростей дельтой.

...
- **UI:** threads expand inline in the feed (accordion under the root), not in a side drawer. The "Тред · N · M новых" button toggles it; a long branch shows the last few replies with a "показать ещё" control, and "свернуть тред" sits **below** the branch. There is **no separate thread composer** — replying in a thread reuses the room's **main composer** in thread mode (context bar "Ответ в тред"), so attachments/stickers/voice work; it sends with `reply_to_message_id = root id` (keeps the branch flat) via the direct mutate path (not the outbox — thread replies don't live in the room's optimistic feed). Thread replies are allowed even where top-level posting is not (comments in a news/read-only channel). Live via the same `message.new` → thread-query invalidation.

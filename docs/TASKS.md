...
- Admin review changes assignment status; a return writes a `task_comments` row (feedback) on the latest submission. Comments are soft-deleted. Both `accept` and `return` require at least one existing submission — 400 otherwise (`"Nothing to accept/return: no submission yet"`); an assignment can't be marked `accepted` out of thin air.

...
- Admin review changes assignment status; a return optionally writes a `task_comments` row (feedback) on the latest submission — the reviewer can leave `comment` empty and no comment row is created. Comments are soft-deleted. Both `accept` and `return` require at least one existing submission — 400 otherwise (`"Nothing to accept/return: no submission yet"`); an assignment can't be marked `accepted` out of thin air.
- Frontend: the return action ("Вернуть / Не принять" on `TrackCard`, `TaskDetail.tsx`) opens a confirmation popup (`Modal` from `Overlay.tsx`) explaining that the submission won't count and the participant can resubmit, with an optional comment textarea — not an inline always-visible field.
- A return notifies the submitter: `notify_task_returned` (bell + native push, kind `task_returned`, see [NOTIFICATIONS.md](NOTIFICATIONS.md)) fires after `assignment.status = "returned"` is flushed — click routes to `/tasks/{task_id}`.
...
- **Meeting.** No in-app scheduling. A member sees "Изучи дневник и задания @partner, по
  необходимости задай вопросы в личных сообщениях." (2nd person); an admin viewing a pair
  they're not in sees "@X и @Y должны изучить дневник и задания друг друга …" (3rd person,
  both members named). The

...
export interface PatchAdminUserBody {
  ...
  email?: string | null
  intake_id?: number
  /** Смена тарифа задним числом (напр. понижение до самого дешёвого, или
   * восстановление обратно). `null` — снять тариф вовсе. Живые проверки
   * (дневники/контакты/задачи/КБ по тарифу) пересчитываются на каждый запрос —
   * кроме уже сданных общих задач, те остаются видны владельцу и админам
   * независимо от тарифа. dm-членства сервер сам синхронизирует с новым
   * кругом в обе стороны (пропавший чат при понижении сам вернётся при
   * восстановлении тарифа). Групповые тариф-чаты — нет, убирать вручную. */
  plan_id?: number | null
}

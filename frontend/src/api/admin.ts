...
export interface PatchAdminUserBody {
  ...
  email?: string | null
  intake_id?: number
  /** Смена тарифа задним числом (напр. понижение до самого дешёвого). `null` —
   * снять тариф вовсе. Живые проверки (дневники/контакты/задачи/КБ по тарифу)
   * пересчитываются на каждый запрос; dm-членства вне нового круга сервер
   * подчищает сам. Групповые тариф-чаты — нет, убирать вручную. */
  plan_id?: number | null
}

...
// viewBox 700×700 — модестно увеличено со старых 660, чтобы у подписи
// освободился запас по радиусу без сжатия остального круга (см. R_LABEL ниже).
export const CX = 350
export const CY = 350
...
// Нет отдельного видимого кольца-ободка — этот радиус только как база отсчёта
// для радиус-пропорциональной задержки пульса (pulseDelay в ExpeditionWheel.tsx).
export const R_HUB_RIM = 62
export const R_CENTER = 96 // кольцо дней Точки Баланса / Финала
export const R_LOCK = 150 // замки стихий (гексаграммы)
export const R_FRAME_IN = 180 // внутренняя граница золотой рамки текущего этапа
export const R_DAY = 210 // диски-луны дней стихий
export const R_FRAME_OUT = 236 // внешняя граница золотой рамки (с запасом на гало сегодняшнего дня)

// Подпись стихии (дата эфира — тем же радиусом, +16px вниз в ExpeditionWheel.tsx)
// — САМОЕ внешнее кольцо, за днями и рамкой, как в исходной (до первой
// переделки) раскладке. Текст рисуется горизонтально БЕЗ поворота: у Огня/
// Земли (верх/низ) это касательная к кругу ориентация — слову есть куда расти
// вбок вдоль обода. У Воздуха/Воды (право/лево) та же горизонтальная
// ориентация ложится ВДОЛЬ радиуса — слову нужен запас ПО РАДИУСУ, а не по
// дуге; отсюда и разнесение с днями/рамкой, а не общий радиус с ними.
export const R_LABEL = 288

// Диагональные лучи (45°/135°/225°/315° — между кардинальными точками
// стихий/замков/подписей, так что ни с чем не пересекаются). Начинаются ПОСЛЕ
// кольца Точки Баланса/Финала (не от самого хаба) — иначе луч задевает те
// луны на выходе из центра; заканчиваются чуть ЗА диском дня (день r=12.5 у
// «сегодня» + гало 6 = макс. охват ~228.5 при R_DAY=210) — луч должен явно
// выходить за луну, а не обрываться перед ней.
export const R_SPOKE_IN = R_CENTER + 16
export const R_SPOKE_OUT = R_FRAME_OUT - 2
